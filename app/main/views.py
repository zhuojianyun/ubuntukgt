...
def insurance(id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('你的保单已经提交！')
        return redirect(url_for('.insurance1', id=post.id, page=-1))
    page = request.args.get('page', 1, type=int)
    if page == -1:
...
@main.route('/post3/<int:id>', methods=['GET', 'POST'])
def insurance1(id):
    post = Post.query.get_or_404(id)
    #insurance = Insurance.query.get_or_404(id)
    form = InsuranceForm1()
    ...
    pagination = post.insurancese.order_by(Insurance.timestamp.asc()).paginate(page, per_page=current_app.config['FLASKY_COMMENTS_PER_PAGE'],error_out=False)
    insurancese = pagination.items
    return render_template('post2.html', posts=[post], form=form,insurancese=insurancese, pagination=pagination)
...
def editii(id):
    ...
    form = InsuranceForm()
    if form.validate_on_submit():
        insurance.insurname = form.insurname.data
        insurance.baodanhao = form.baodanhao.data
        insurance.toubaoriqi = form.toubaoriqi.data
        insurance.shengxiaoriqi = form.shengxiaoriqi.data
        insurance.baodanzhuangtai = form.baodanzhuangtai.data
        insurance.jiaofeifangshi = form.jiaofeifangshi.data
        insurance.jiaofeiqi = form.jiaofeiqi.data
        insurance.baoxianqijian = form.baoxianqijian.data
        insurance.shixiaoriqi = form.shixiaoriqi.data
        insurance.baofei = form.baofei.data
        ...
        insurance.jjname = form.syname.data
